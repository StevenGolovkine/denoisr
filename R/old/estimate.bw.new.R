...
estimate.bw.new <- function(data, pct=0.1, sigma=NULL, s_0=NULL, L_0=NULL, t0=0, l=2, K='epanechnikov', 
  ...
  # Step 3 - Estimate s_0
  substract_element <- function(v, i, t0) (v[2*i + 1 + t0] - v[1 + t0])**2 - (v[i + 1 + t0] - v[1 + t0])**2
  ...
  if(est_list['s_0']){
    if(is.null(s_0)){
      two_log_two <- 2*log(2)
      first_part <- S_N_0 %>% 
        map_dbl(~ substract_element(.x$x, 2, t0 = t0)) %>% 
        mean() %>% 
        max(c(., 1e-5)) %>%
        log()
      second_part <- S_N_0 %>% 
        map_dbl(~ substract_element(.x$x, 1, t0 = t0)) %>% 
        mean() %>% 
        max(c(., 1e-5)) %>%
        log()
  ...
  # Step 4 - Estimate L_0
  substract_first_element <- function(v, i) (v[2*(i+1)] - v[1])**2 - (v[i+1] - v[1])**2
