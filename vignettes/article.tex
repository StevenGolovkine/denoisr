%% $Id: article.Rnw,v 0.1 2020/01/08 $

%\VignetteIndexEntry{An Introduction to the SmoothCurves package}
%\VignetteKeywords{nonparametric, kernel, functional data, smoothing}
%\VignettePackage{SmoothCurves}
...
\section{Empirical Illustration} \label{sec:illustrations}

This section introduces the main features of the package \pkg{SmoothCurves} 
employing some simulated datasets and the Canadian daily temperature data 
\citep{ramsay_functional_2005}, that are available e.g. in the \pkg{fda} package
\citep{fda_2018}. It contains daily observations of temperature (in \°C) for $N = 35$
Canadian weather stations averaged over the years 1960 to 1994.

From within \proglang{R}, the latest version of \pkg{SmoothCurves} can be 
installed from the Comprehensive \proglang{R} Archive Network (CRAN) 
(\textcolor{red}{maybe one day}) using the following
%
\begin{Schunk}
\begin{Sinput}
R> install.packages("SmoothCurves")
\end{Sinput}
\end{Schunk}
%
and the latest development version of the package can be installed from Github
using the following
%
\begin{Schunk}
\begin{Sinput}
R> devtools::install_github("StevenGolovkine/SmoothCurves")
\end{Sinput}
\end{Schunk}
%

Once the package has been installeed, it can be attached by typing:
%
\begin{Schunk}
\begin{Sinput}
R> library(SmoothCurves)
\end{Sinput}
\end{Schunk}
%
...
\begin{Schunk}
\begin{Sinput}
R> help(package="SmoothCurves")
\end{Sinput}
\end{Schunk}
%

\subsection{A note on the format of the data}
...
Y_{n, j} = X_n(T_{n, j}) + \varepsilon_{n, j}, \quad 1 \leq n \leq N,
\end{equation}
%
where $T_{n, j} \sim f$, with $f$ a probability density function with support $[0, 1]$,
and $\varepsilon_{n, j} \sim \mathcal{N}(0, \sigma^2)$ are independant variables. 
Denote by $H$ the regularity of $X$ and define $X$ as a standard Brownian motion
($H = 0.5). The \code{fractional_brownian} data from the package \pkg{SmoothCurves}
is used. 
...
\begin{Schunk}
\begin{Sinput}
R> data("fractional_brownian", package = "SmoothCurves")
\end{Sinput}
\end{Schunk}
%

The smoothing of the curves can be easyly performed using one line
%
\begin{Schunk}
\begin{Sinput}
R> smooth_data <- smooth.curves(fractional_brownian)
R> smooth_data$parameter
\end{Sinput}
\begin{Soutput}
$sigma
[1] 0.07080734
...
We discuss alternative choices for implementation and inference further below, 
but first we plot the results obtained above. 
%
\begin{Schunk}
\begin{Sinput}
R> df <- tibble(t = fractional_brownian[[1]]$t,
+               x = fractional_brownian[[1]]$x,
+               x_true = fractional_brownian[[1]]$x_true,
+               x_hat = smooth_data$smooth[[1]]$x)
R> ggplot(df) +
+    geom_point(aes(x = t, y = x), alpha = 0.5) +
+    geom_line(aes(x = t, y = x_true), col = 'red') +
+    geom_line(aes(x = t, y = x_hat), col = 'blue') +
+    labs(x = '', y = '') +
+    theme_custom()
\end{Sinput}
\end{Schunk}
